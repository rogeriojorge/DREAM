...
def basicSetting(ds, nr=NR, pMax=PMAX, Np=NP, isotropic=False, activated=None, out_init=None, superthermal=False):
    """
    ...
    # Temperature
    rT0, T0 = Tokamak.getInitialTemperatureProfile(nr)  # Initial profile

    # Initial densities
    if activated == 'DT':
        nD = 0.5 * Tokamak.getInitialDensityProfile(nr)[1]
        nT = 0.5 * Tokamak.getInitialDensityProfile(nr)[1]
    else:
        nD = 1. * Tokamak.getInitialDensityProfile(nr)[1]

    # Other parameters
    tau0 = 1 / Tokamak.tau_w  # inverse wall time

    # Set tokamak specifications
    Tokamak.setRadialGrid(ds, nr=nr)
    ...
    rj, j = Tokamak.getInitialCurrentDensityProfile(nr)
    Ip = Tokamak.Ip
    ...
    ds.eqsys.E_field.setBoundaryCondition(EField.BC_TYPE_SELFCONSISTENT, inverse_wall_time=tau0, R0=Tokamak.R0)
...
def injectMaterial(ds, nD, nNe, cD=0, cNe=0, do_init=None):
    if nD > 0:
        r_nD = np.linspace(0, Tokamak.a, (NR-1)*20+1)[1:]
        if cD == 0:
            profile = np.ones((NR-1)*20+1)[1:]
            n_nD = nD * profile
        else:
            profile = (1 + np.tanh(cD * ((r_nD / Tokamak.a) - .5)))
    ...
    if nNe > 0:
        r_nNe = np.linspace(0, Tokamak.a, (NR-1)*20+1)[1:]
        if cNe == 0:
            profile = np.ones((NR-1)*20+1)[1:]
            n_nNe = nNe * profile
        else:
            profile = (1 + np.tanh(cNe * ((r_nNe / Tokamak.a) - .5)))
...
def runSimulation(nD_inj, nNe_inj, cD_inj=0, cNe_inj=0, isotropic=False, superthermal=False, activated=None, outputDir='.'):
    # Time parameters
    tmax_TQ_temp = Tokamak.tmax_TQ_max  # Maximum time for temporary thermal quench simulation
    tmax_CQ = Tokamak.tmax_CQ  # Total simulation time
    ...
    ds, do_init = basicSetting(ds, isotropic=isotropic, superthermal=superthermal, out_init=out_init, activated=activated)
